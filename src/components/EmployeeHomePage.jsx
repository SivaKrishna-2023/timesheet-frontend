...
const EmployeeHomePage = ({ submissions, setSubmissions, employeeId = 'MTL1021' }) => {
  ...
  useEffect(() => {
    const fetchSubmissions = async () => {
      try {
        const response = await axios.get(`timesheet-backend-dyejhvfjaaakg9ht.germanywestcentral-01.azurewebsites.net/api/timesheets/list/${employeeId}`);
  ...
  const handleDeleteTimesheet = async (id) => {
    try {
      await axios.delete(`timesheet-backend-dyejhvfjaaakg9ht.germanywestcentral-01.azurewebsites.net/api/timesheets/delete/${id}`);
